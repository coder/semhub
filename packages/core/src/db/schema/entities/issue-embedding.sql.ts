...
export const issueEmbeddings = pgTable(
  ...
  (table) => [
    uniqueIndex("issue_embeddings_issue_id_idx").on(table.issueId),
    index("issue_embeddings_sync_status_idx").on(table.embeddingSyncStatus),
    ...
    index("issue_embeddings_embedding_idx").using(
      "hnsw",
      table.embedding.op("vector_cosine_ops"),
    ),
    index("issue_embeddings_status_generated_at_idx").on(
      table.embeddingSyncStatus,
      table.embeddingGeneratedAt,
    ),
    index("issue_embeddings_null_idx")
      .on(table.issueId)
